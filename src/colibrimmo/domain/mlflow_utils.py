...
def mlflow_log_pandas(df: pd.DataFrame, artifact_path: str, local_path: str) -> None:
    """
    ...
    logger.info(f"mlflow_log_pandas: {local_path}")
...
def mlflow_log_figure(fig, artifact_path: str, local_path: str) -> None:
    """
    Save a plotly figure in a temporary directory.
    Log the temporary directory within the mlflow current run.
    ...
    mlflow.log_artifact(local_path=file_path, artifact_path=artifact_path)
    logger.info(f"mlflow_log_pyplot_figure: {local_path}")
...
def _find_existing_run(
    ...
    all_run_infos = reversed(mlflow_client.list_run_infos(experiment_id))
...
def get_run(
    ...
    existing_run = _find_existing_run(
        mlflow_client, entry_point, parameters, git_commit
    )
    if existing_run:
        return existing_run
    submitted_run = mlflow.run(".", entry_point=entry_point, parameters=parameters)
