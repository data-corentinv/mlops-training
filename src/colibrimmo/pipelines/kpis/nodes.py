...
def merge_zone_ads(ads: pd.DataFrame, cities: pd.DataFrame, postalcodes: pd.DataFrame, insee: pd.DataFrame, departements: pd.DataFrame, regions: pd.DataFrame) -> pd.DataFrame:
    regions = regions.filter(["insee_code", "name"]).rename(columns={"insee_code": "region_insee_code_r", "name": "region_name"})
    departements = departements.filter(["insee_code", "name", "region_insee_code"]).rename(columns={"insee_code": "department_insee_code_r", "name": "department_name"})
    postalcodes = postalcodes.rename(columns={"postal_code": "postal_code_r"})
    cities = cities.filter(["insee_code", "name", "department_insee_code"]).rename(columns={"name": "city_name"})
    insee = insee.filter(["city_insee_code", "postal_code"])

    departements = merge_many_to_one(departements, regions, "region_insee_code", "region_insee_code_r")
    cities = merge_many_to_one(cities, departements, "department_insee_code", "department_insee_code_r")
